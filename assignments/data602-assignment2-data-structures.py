...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    other_temp = [a, b, c, d, e]
    temperatures.extend(other_temp)
    temperatures = sorted(temperatures, reverse=True)
    
    # get a sample of temperatures
    samples = temperatures[4::5]
    copy_of_samples = samples[:]
    samples = sorted(samples) # sorted in ascending by default
...
def exercise10(sentence):
    ...
    sentence = sentence.replace(" ", "_").swapcase() # replace space with underscore and swapcase
